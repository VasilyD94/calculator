# 📋 ТЕХНИЧЕСКОЕ ЗАДАНИЕ: CalcBox (calc-box.ru) v3.0

> **Версия:** 3.0 (MVP завершён, февраль 2026)
> **Домен:** [calc-box.ru](https://calc-box.ru)
> **Текущий статус:** 13 калькуляторов в продакшене
...
| Сервис | Назначение |
|--------|------------|
| Timeweb VPS (72.56.97.130) | Хостинг (Ubuntu 24, Nginx → Next.js:3001, PM2) |
| Let's Encrypt + certbot | SSL (автообновление) |
| REG.ru | Домен calc-box.ru |
| GitHub | Репозиторий |
| Яндекс.Метрика (106583944) | Аналитика + Вебвизор |
| Яндекс.Вебмастер | Индексация |
| РСЯ | Монетизация (Фаза 2) |
...
- [x] Инфраструктура проекта (Next.js 15, shadcn/ui, Tailwind)
- [x] UI-компоненты (ValueSlider, GenderToggle, ActivitySelector, FormulaComparison)
- [x] Калькуляторы Tier 1 (5 шт): калории, ИМТ, БЖУ, дата родов, срок беременности
- [x] Калькуляторы Tier 2 (5 шт): овуляция, идеальный вес, дефицит калорий, норма воды, процент жира
- [x] Калькуляторы Tier 3 (3 шт): базовый метаболизм, пульс для тренировок, белок для спортсменов
- [x] SEO-тексты 1500+ слов для каждой страницы
- [x] JSON-LD разметка (WebApplication, FAQPage, HowTo, BreadcrumbList)
- [x] Деплой на Timeweb VPS (Nginx + PM2)
- [x] SSL через Let's Encrypt
- [x] Яндекс.Вебмастер + Метрика

### Месяц 3-4 — Следующий этап
- [ ] Новые калькуляторы (менструальный цикл, набор веса, тип телосложения, дети)
- [ ] Контент-маркетинг (Дзен, Яндекс.Кью)
- [ ] Google Search Console
- [ ] Подключение РСЯ (при 100+ уник/день)

### Месяц 5-6
- [ ] Оставшиеся калькуляторы до 25
- [ ] Оптимизация под топ-10
