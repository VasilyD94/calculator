...
Хостинг: Timeweb VPS
IP: 72.56.97.130
ОС: Ubuntu 24 (6.8.0-90-generic)
RAM: 3.8 GB
Диск: 38 GB (свободно ~21 GB)
```
...
```
Node.js: v20.20.0
PM2: 6.0.14 (автозапуск при перезагрузке через systemd)
...
```
/var/www/calculator/        — корень проекта
```
...
Симлинк: /etc/nginx/sites-enabled/calculator

Схема: Nginx (порт 80) → proxy_pass → Next.js (порт 3001)

ВАЖНО: Порт 3000 занят Docker-контейнером, поэтому приложение работает на порту 3001.
PM2 запущен с переменной PORT=3001.
```
...
### Деплой по шагам (если нужно вручную)

```bash
# 1. Подключиться к серверу
ssh root@72.56.97.130
...
# Проверить статус
pm2 status
pm2 logs calculator --lines 20 --nostream
```
...
pm2 logs calculator --lines 50 --nostream
